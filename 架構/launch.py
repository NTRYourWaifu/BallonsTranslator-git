...
def main():
    ...
    # ****** 調整這裡的數字來設置顯存上限 ******
    # 例如，0.5 表示限制使用 50% 的 GPU 顯存
    # 例如，0.75 表示限制使用 75% 的 GPU 顯存
    # 必須在所有 CUDA 初始化之前設定，所以放在 main() 最前面
    MEMORY_FRACTION = 0.8
    GPU_INDEX = 0  # 如果您只有一張卡，通常是 0

    import torch
    if torch.cuda.is_available():
        try:
            torch.cuda.set_per_process_memory_fraction(MEMORY_FRACTION, device=GPU_INDEX)
            print(f"PyTorch 顯存限制設置為 {MEMORY_FRACTION * 100:.0f}% 在 GPU {GPU_INDEX} 上。")
        except Exception as e:
            print(f"設置 PyTorch 顯存限制失敗: {e}")
