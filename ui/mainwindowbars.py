...
from qtpy.QtCore import Qt, Signal, QPoint, QByteArray
from qtpy.QtGui import QMouseEvent, QKeySequence, QActionGroup, QIcon, QPixmap
...
from ui.custom_widget import OcrStatsBar
...
# ── 左側四個控制按鈕的 SVG 圖示 ─────────────────────────────
_SVG_PAUSE = '''<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  <rect x="5" y="4" width="4" height="16" rx="1" fill="#cccccc"/>
  <rect x="15" y="4" width="4" height="16" rx="1" fill="#cccccc"/>
</svg>'''

_SVG_STOP = '''<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  <rect x="4" y="4" width="16" height="16" rx="2" fill="#cccccc"/>
</svg>'''

_SVG_RESUME_HERE = '''<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  <polygon points="6,4 20,12 6,20" fill="#cccccc"/>
  <line x1="4" y1="4" x2="4" y2="20" stroke="#cccccc" stroke-width="2.5" stroke-linecap="round"/>
</svg>'''

_SVG_RESUME_LAST = '''<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  <path d="M12 4 A8 8 0 1 0 20 12" fill="none" stroke="#cccccc" stroke-width="2.5" stroke-linecap="round"/>
...
class StickyMenu(QMenu):
    """勾選 checkable action 時不關閉選單，點選單外才關閉"""
...
class LeftBar(Widget):
    ...
    def __init__(self, mainwindow, *args, **kwargs) -> None:
        ...
        self.stopBtn.setIcon(_svg_icon(_SVG_STOP, btn_size))
        self.stopBtn.setIconSize(self.stopBtn.size())
        self.stopBtn.setToolTip(self.tr('停止'))

        self.resumeHereBtn = QPushButton()
        self.resumeHereBtn.setFixedSize(btn_size, btn_size)
...
class TitleBar(Widget):
    ...
    def __init__(self, parent, *args, **kwargs) -> None:
        ...
        runMenu = StickyMenu(self.runToolBtn)
...
class BottomBar(Widget):
    ...
    def __init__(self, mainwindow: QMainWindow, *args, **kwargs) -> None:
        ...
        self.ocr_stats_bar = OcrStatsBar(self)
        self.hlayout.addWidget(self.ocr_stats_bar)
